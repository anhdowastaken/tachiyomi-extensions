package eu.kanade.tachiyomi.extension.vi.beeng
...
class Beeng : ParsedHttpSource() {

    override val name = "Beeng"

    override val baseUrl = "https://beeng.net"
    ...
    override fun popularMangaSelector() = "div.manga-list div.tit"

    override fun latestUpdatesSelector() = "div.manga-list div.tit"

    override fun popularMangaRequest(page: Int): Request {
        if (page == 1) {
            return GET("$baseUrl/noi-bat", headers)
        } else {
            val number = (page-1)*30
            return GET("$baseUrl/noi-bat/$number", headers)
        }
    }

    override fun latestUpdatesRequest(page: Int): Request {
        if (page == 1) {
            return GET("$baseUrl/truyen-moi", headers)
        } else {
            val number = (page-1)*30
            return GET("$baseUrl/truyen-moi/$number", headers)
        }
    }
...
//        var temp = "$baseUrl/https://beeng.net/autosearch/"
...
//        return GET(url.toString().replace("m.", ""), headers)
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        val manga = SManga.create()
        manga.author = document.select("div.manga-detail ul > li:nth-child(4) > span:nth-child(2)").first().text()
        manga.artist = manga.author
        manga.genre = document.select("div.manga-detail ul > li:nth-child(7) > a").joinToString { it.text() }
        manga.description = document.select("div.manga-summary").first().text()
        manga.status = document.select("div.manga-detail ul > li:nth-child(9)")?.text().orEmpty().let { parseStatus(it) }
        manga.thumbnail_url = document.select("div.manga-detail img").first().attr("src")
        return manga
    }

    fun parseStatus(status: String) = when {
        status.contains("Đang thực hiện") -> SManga.ONGOING
        status.contains("Đã hoàn thành") -> SManga.COMPLETED
        else -> SManga.UNKNOWN
    }

    override fun chapterListSelector() = "div.manga-chapter > ul > li.active > ul > li.active > ul > li"
    ...
    override fun chapterFromElement(element: Element): SChapter {
        ...
        chapter.date_upload = element.select("span.date").first()?.text()?.let {
    ...
    override fun pageListParse(document: Document): List<Page> {
        ...
        document.select("div#image-load img").forEachIndexed { i, e ->
